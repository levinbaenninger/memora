...
export const getOutboundLinksResponseDtoSchema = z.array(
...
export const getOutboundLinks = authorized
  .input(getOutboundLinksRequestDtoSchema)
  .output(getOutboundLinksResponseDtoSchema)
  .errors({ NOT_FOUND: {} })
  .handler(async ({ context, input, errors }) => {
    ...
    if (!sourceNote || (!input.includeArchived && sourceNote.archivedAt)) {
    ...
    const where: SQL[] = [
      ...
      eq(noteLinks.sourceNoteId, input.id),
    ...
    return getOutboundLinksResponseDtoSchema.parse(
